...
_EXT_RE = re.compile(r".+\.(png|jpg|jpeg|gif|svg|pdf|txt|rst|md|html|css|js|json|yaml|yml|csv|parquet|ipynb)$")
_NUM_RE = re.compile(r"^\d+$")
_VER_RE = re.compile(r"^\d+(\.\d+)+$")          # 1.8.0, 0.955, 3.10.19
_PATHISH_RE = re.compile(r"[\\/]|::")           # / \ or python :: style
_BAD_PUNCT_ONLY_RE = re.compile(r"^[\W_]+$")    # only punctuation/underscores

def make_tokenizer(pattern: str):
    ...
    def _tok(text: str) -> List[str]:
        ...
        base = [t.lower() for t in rx.findall(text)]
        out: List[str] = []
        ...
        for tok in base:
            ...
            for sep in ("-", "_", "."):
                if sep in tok:
                    out.extend([p for p in tok.split(sep) if p])
        ...
        for t in out:
            ...
            if len(t) <= 1:
                continue
            if _BAD_PUNCT_ONLY_RE.match(t):
                continue
            if _PATHISH_RE.search(t):
                continue
            if _EXT_RE.match(t):
                continue
            if _NUM_RE.match(t) or _VER_RE.match(t):
                continue
            filtered.append(t)
...
def make_tokenizer(pattern: str):
    ...
    def _tok(text: str) -> List[str]:
        ...
        for tok in base:
            out.append(tok)  # keep full token

            # also add split parts for matching natural queries
            for sep in ("-", "_", "."):
                if sep in tok:
                    parts = [p for p in tok.split(sep) if p]
                    out.extend(parts)

        # optional: drop 1-char tokens (usually noise)
        out = [t for t in out if len(t) > 1]
        return out

    return _tok

TOKEN_PATTERN = r"[A-Za-z0-9_]+(?:\.[A-Za-z0-9_]+)*|[A-Za-z0-9_]+(?:-[A-Za-z0-9_]+)+|[A-Za-z0-9_]+"
tokenize = make_tokenizer(TOKEN_PATTERN)
	
filter=[
        Namespace("source", ["kb"], []),            # allow_list=["kb"]
        Namespace("lang", ["en"], []),              # allow_list=["en"]
        # Namespace("doctype", [], ["invoice"])     # deny_list=["invoice"]
    ],

    # numeric restricts
    numeric_filter=[
        NumericNamespace(name="year", value_int=2023, op="GREATER_EQUAL"),
        NumericNamespace(name="chunk_len", value_int=1200, op="LESS_EQUAL"),
    ],
)
...
Vertex AI Search
Supports managed document ingestion, replacement, and deletion within a datastore; document updates are applied via re-indexing, with no native document-level versioning.
	•	Vertex AI RAG Engine
Supports document ingestion and deletion within a corpus; does not support in-place updates or versioning—documents are immutable and must be deleted and re-ingested.
	•	Vertex AI Vector Search
Supports explicit ingestion, in-place updates (upserts), deletion, and user-defined versioning through direct datapoint management
...
Supports rich, expressive metadata filtering using query-time filter expressions across all indexed filterable fields (AND/OR/NOT, ranges, field comparisons).
	•	Vertex AI Vector Search
Supports structured, exact-match metadata

 filtering via pre-indexed restrict namespaces and numeric constraints; no free-form query expressions.
	•	Vertex AI RAG Engine
