def histogram_plot_binned(series, title, max_bins=50):
    ...
    bin_edges_linear = np.expm1(bin_edges_log)

    bin_widths = np.diff(bin_edges_linear)
    bin_centers = bin_edges_linear[:-1] + bin_widths / 2

    df_bins = pd.DataFrame({
        "bin_center": bin_centers,
        "bin_width": bin_widths,
        "count": counts,
        "bin_range": [f"{round(left,2)}–{round(right,2)}" 
            for left, right in zip(bin_edges_linear[:-1], bin_edges_linear[1:])]
    })
    ...
    fig.add_bar(
        ...
        y=df_bins["count"],
        width=df_bins["bin_width"],
        hovertext=df_bins["bin_range"],
        marker=dict(line=dict(width=0.5, color="black"))
    )
    fig.update_xaxes(title="Value (original scale)")
