def histogram_plot_binned(series, title, max_bins=50):
    ...
    df_bins = pd.DataFrame({
        "bin_center_log": bin_centers_log,
        "count": counts,
        "bin_range": [
    ...
    fig = px.bar(
        ...
        title=title,
        labels={"bin_center_log": "Value (log scale)", "count": "Count"}
    )

    # set x axis to log
    fig.update_xaxes(type="log", tickvals=bin_centers_log,
        ticktext=[f"{round(np.expm1(c),2)}" for c in bin_centers_log]
    )
