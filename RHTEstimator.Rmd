...
def calculate_information_value(bin_object, x_var):
    """
    Computes Information Value (IV) for a variable using WoE transformation.
    """
    try:
        woe_summary = bin_object.woe_summary[[x_var, 'WoE_' + x_var, 'Good', 'Bad']]
        
        # Compute %Good and %Bad
        woe_summary['%Good'] = woe_summary['Good'] / woe_summary['Good'].sum()
        woe_summary['%Bad'] = woe_summary['Bad'] / woe_summary['Bad'].sum()
        
        # Compute IV
        woe_summary['IV'] = (woe_summary['%Good'] - woe_summary['%Bad']) * woe_summary['WoE_' + x_var]
        information_value = woe_summary['IV'].sum()

        return information_value

    except Exception as e:
        print(f"⚠️ Could not calculate IV for {x_var}: {e}")
        return None
...
def process_variable(x_var, base):
    ...
    bin_object = Binning(y_var, n_threshold=50, y_threshold=2, p_threshold=0.02, sign=False)
    ...
    try:
        ...
        iv_value = calculate_information_value(bin_object, x_var)
