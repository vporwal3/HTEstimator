...
substrings_to_find = ['SALE', 'RETURN', 'CHGBCK']
regex_pattern = '|'.join(re.escape(s) for s in substrings_to_find)

# Use list comprehension to get matching columns
matched_columns_list = [col for col in base.columns if re.search(regex_pattern, col, re.IGNORECASE)]
...
# Assign special code to features where prd_d < 2020
df.loc[df['prd_d'].dt.year < 2020, feature_cols] = special_code
