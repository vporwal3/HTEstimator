...
def box_plot_with_extremes(series, title):
    ...
    # outliers
    extreme_points = series[(series < series.quantile(0.01)) | (series > series.quantile(0.99))]
    ...
    fig.add_trace(go.Scatter(
        y=extreme_points,
        mode="markers",
        name="Extreme Values (1–99%)",
        marker=dict(color="red", size=6, symbol="circle")
    ))

    fig.update_layout(title=title, yaxis_title="Value", margin=dict(l=10,r=10,t=40,b=10))
    return fig

def violin_plot_kde(series, title, max_points=500):
    series = series.dropna()
    series = series[series >= 0]  # drop negatives if any
    kde = gaussian_kde(series)
    x_grid = np.linspace(series.quantile(0.01), series.quantile(0.99), max_points)
    y_density = kde(x_grid)

    fig = go.Figure()

    # mirror the density for violin
    fig.add_trace(go.Scatter(
        x=y_density,
        y=x_grid,
        mode="lines",
    ...
    # median and IQR overlays
    q1 = series.quantile(0.25)
    q3 = series.quantile(0.75)
    median = series.median()

    fig.add_shape(type="line", x0=-y_density.max(), x1=y_density.max(), y0=median, y1=median,
                  line=dict(color="black", width=2, dash="dash"), name="Median")
    fig.add_shape(type="rect", x0=-y_density.max(), x1=y_density.max(), y0=q1, y1=q3,
                  line=dict(color="black", width=1), fillcolor="lightgrey", opacity=0.2)

    fig.update_xaxes(title="Density", showgrid=True)
...
from plots.plot_factory import box_plot_with_extremes, violin_plot_kde
...
def register_callbacks(app, df, numeric_vars):
    ...
    def update_dashboard(variable):
        ...
        box_current = box_plot_with_extremes(df_current[variable], title=f"Box Plot – Snapshot {current_time.strftime('%b %Y')}")

        violin_overall = violin_plot_kde(df[variable], title="Violin Plot – Overall Data")
        violin_current = violin_plot_kde(df_current[variable], title=f"Violin Plot – Snapshot {current_time.strftime('%b %Y')}")

        return box_overall, box_current, violin_overall, violin_current
