...
def build_layout(variable_options):
    return dbc.Container([
        ...
        dbc.Row([
            dbc.Col([
                html.H5(id="missing-title-total"),
                html.Div(id='summary-total')
            ], width=6),
            dbc.Col([
                html.H5(id="missing-title-current"),
        ...
        dbc.Row([
            dbc.Col([
                html.H5(id="numeric-title-total"),
                html.Div(id='summary-total-numeric')
            ], width=6),
            dbc.Col([
                html.H5(id="numeric-title-current"),
                html.Div(id='summary-current-numeric')
            ], width=6)
        ], className="mb-4"),
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Dropdown(id="metric-dropdown", placeholder="Select metric to plot"),
...
def register_callbacks(app, df, numeric_vars):
    ...
    def update_dashboard(variable, selected_metric):
        if variable is None:
            return "", "", "", "", [], None, "", "", "", "", {}
        ...
        snapshot_date = df["report_date"].max().strftime("%b %Y")

        # Plot metrics
        available_metrics = numeric_results["time_series"]["Metric"].unique()
        metric_options = [{"label": m, "value": m} for m in available_metrics]
        metric = selected_metric if selected_metric in available_metrics else available_metrics[0]
        ...
        return (
            ...
            line_plot(numeric_results["time_series"], metric)
        )
