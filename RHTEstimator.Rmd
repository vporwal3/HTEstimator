...
def histogram_plot_binned(series, title, max_bins=50):
    ...
    # bin positive values on log scale
    log_series = np.log10(positive_series)

    lower = log_series.quantile(0.01)
    upper = log_series.quantile(0.99)

    if lower == upper:
        lower = log_series.min()
        upper = log_series.max() + 1e-3

    bin_edges_log = np.linspace(lower, upper, max_bins + 1)
    counts, _ = np.histogram(log_series, bins=bin_edges_log)

    bin_centers_log = (bin_edges_log[:-1] + bin_edges_log[1:]) / 2
    bin_centers_linear = 10 ** bin_centers_log

    bin_ranges = [
        f"{round(10**left,2)}–{round(10**right,2)}"
        for left, right in zip(bin_edges_log[:-1], bin_edges_log[1:])
    ]
...
def histogram_plot_binned(series, title, max_bins=50):
    ...
    fig.update_xaxes(title="Value (Original Scale)", showgrid=True)
    fig.update_yaxes(title="Count")
    fig.update_layout(
        title=title,
        bargap=0.1,
