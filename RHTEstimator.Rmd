...
def violin_plot_kde(series, title, max_grid=500):
    """
    Builds a violin plot from a server-side KDE and overlays median + IQR.
    Uses only 500 grid points to send to frontend.
    """
    series = series.dropna()
    # only consider positive for KDE if appropriate
    series = series[series >= 0]

    if len(series) < 10:
        # too small to estimate KDE
        return go.Figure()

    # build KDE on the server
    kde = gaussian_kde(series)

    # use 1–99% to avoid extreme tails
    lower = series.quantile(0.01)
    upper = series.quantile(0.99)
    grid_y = np.linspace(lower, upper, max_grid)
    density = kde(grid_y)

    fig = go.Figure()

    # positive side of violin
    fig.add_trace(go.Scatter(
        x=density,
        y=grid_y,
        mode="lines",
