...
def compare_variable_across_windows(deposit_df, variable, windows=None):
    """
    Generate KDE, boxplot, and segment-wise boxplot for a given variable across two time windows.

    Parameters:
    - deposit_df: DataFrame containing deposit data
    - variable: str, the column name of the variable to analyze
    - windows: dict with format {"label": (start_year, end_year)} — optional
    """
    if windows is None:
        windows = {
            "2018–2021": (2018, 2021),
    ...
    df = df[df['window'].notnull() & df[variable].notnull()].copy()
