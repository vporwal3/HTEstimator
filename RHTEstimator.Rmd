ldef histogram_plot_binned(series, title, max_bins=50):
    import numpy as np
    import pandas as pd
    import plotly.express as px

    # keep zeros, drop negatives only
    series = series.dropna()
    series = series[series >= 0]

    # log transform (preserve zeros)
    log_series = np.log1p(series)

    # calculate bin edges on log scale
    lower = log_series.quantile(0.01)
    upper = log_series.quantile(0.99)

    # fallback if identical (avoid zero-width bins)
    if lower == upper:
    ...
    # bin counts
    counts, _ = np.histogram(log_series, bins=bin_edges)

    # bin centers back in original scale
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    bin_centers_original = np.expm1(bin_centers)

    df_bins = pd.DataFrame({
        "bin_center": bin_centers_original,
        "count": counts,
        "bin_range": [
            f"{round(np.expm1(left),2)}–{round(np.expm1(right),2)}"
